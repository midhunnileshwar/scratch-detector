...
# CONFIG
# -------------------------------------------------
st.set_page_config("KITE Forensics Master V2.0 ", "🛡️", layout="wide")
...
# -------------------------------------------------
# FIXED PROJECT EXTRACTOR (CORE FIX)
# -------------------------------------------------
    ...
    try:
        ...
        with zipfile.ZipFile(file_obj) as z:
            ...
            if "project.json" not in z.namelist():
                return None  # Not a Scratch/PictoBlox project
            ...
            for t in targets:
                ...
                # PictoBlox format (list)
                elif isinstance(blocks, list):
                    for b in blocks:
                        if isinstance(b, dict) and not b.get("shadow"):
                            opcodes.append(b.get("opcode", "unknown"))

                # blocks == None → valid, but empty (do nothing)

    except Exception:
        return None

    # IMPORTANT: Always return if project.json exists
    return {
...
images = {}     # dict[owner] = list of posters
    ...
    with st.spinner("Processing files..."):
        ...
        def process_file(name, data):
            ...
            if ext in ["sb3","p3b"]:
                ...
                if logic is not None:   # ✅ CRITICAL FIX
                    projects.append({
                        "owner": owner,
                        "hash": sha256(data),
                        **logic
    ...
    if len(projects) < 2:
        ...
        for a, b in combinations(projects, 2):
            if a["hash"] == b["hash"]:
                ...
                if sim > 85:
                    st.warning(
                        f"⚠️ {a['owner']} vs {b['owner']} — {sim:.1f}% similarity "
                        f"(Sprites: {a['sprites']} vs {b['sprites']})"
                    )

# -------------------------------------------------
# POSTER ANALYSIS (UNCHANGED – ALREADY PERFECT)
# -------------------------------------------------
with tabs[1]:
    ...
    if len(owners) < 2:
        ...
        for s1, s2 in combinations(owners, 2):
            for i1 in images[s1]:
                for i2 in images[s2]:
                    sim = cv2.compareHist(
                        i1["hist"], i2["hist"], cv2.HISTCMP_CORREL
                    ) * 100
                    if sim > 80:
